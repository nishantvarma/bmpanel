...
static const int verbosity_map[] = {LOG_ERRORS, LOG_WARNINGS, LOG_MESSAGES,
                                    LOG_INFOS, LOG_DEBUGS};
...
void log_set_verbosity(int level) { log_verbosity = level; }

void log_console_callback(int level, const char *msg) { printf("%s\n", msg); }
...
void log_file_callback(int level, const char *msg) {
    ...
    static const char *log_file_params[] = {"[EE]", "[WW]", "[MM]", "[II]",
                                            "[DB]"};
